...
void checkSerialCmd(){
  if (Serial.available()){
    ...
    mapped_code = strtoull(data.c_str(), nullptr, 16);
    Serial.print("the custom code has changed: 0x");
    Serial.println( mapped_code, HEX);
  }
}

void conductButtonActions(){
  
  if (digitalRead(ledPwrBtn)==LOW) {
    // sendLedPower();
    sendTVPower();
    delay(500);
  }
  if (digitalRead(ledCycleBtn)==LOW) {
    Serial.println("Magic Button pressed");
    sendLedSignal(mapped_code);
    delay(500);
  }
}
